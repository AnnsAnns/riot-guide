...
<Aside type="tip"> 
Do not be afraid to ask for help e.g. in [our forum](https://forum.riot-os.org/) or [Matrix chat](https://matrix.to/#/#riot-os:matrix.org).

We are happy to help you get started with RIOT, and we appreciate your feedback.
</Aside>

### Choosing an Operating System for the Development PC

<Tabs syncKey="os">
    <TabItem label="Linux (Recommended)">
...
<Tabs syncKey="os">
    ...
    <Code 
        code={"sudo apt install make gcc-multilib python3-serial wget unzip git openocd gdb-multiarch esptool podman-docker clangd clang"}
        lang='bash'
        title='Ubuntu command to install required packages'
    />
    </TabItem>
    <TabItem label="Arch Linux">
        <Code 
        code={"sudo pacman -S make gcc-multilib python-pyserial wget unzip git openocd gdb esptool podman-docker clang"}
        lang='bash'
        title='Arch Linux command to install required packages'
    />
    </TabItem>
</Tabs>
